...
const PALETTE_CRYSTAL_THEMES: Record<string, { base: string; emissive: string; m3: string; m7: string; m30: string }> = {
...
export function ThreeStreakCrystal({ streak, size = 180 }: ThreeStreakCrystalProps) {
  ...
  useEffect(() => {
    ...
    const pointLight = new THREE.PointLight(new THREE.Color(baseHex), 3.5, 20);
    ...
    const wireGeo = new THREE.OctahedronGeometry(1.62, 0);
    ...
    const animate = () => {
      ...
      // Smooth spin + interactive tilt
      crystal.rotation.y += (targetRotY + t * 0.6 - crystal.rotation.y) * 0.05;
      crystal.rotation.x += (targetRotX + Math.sin(t * 0.8) * 0.2 - crystal.rotation.x) * 0.05;
      crystal.position.y = Math.sin(t * 1.5) * 0.12;

      wire.rotation.copy(crystal.rotation);
      wire.position.copy(crystal.position);

      embers.rotation.y = -t * 0.3;
      embers.rotation.x = Math.sin(t * 0.5) * 0.2;
