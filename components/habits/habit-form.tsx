...
interface HabitFormProps {
  ...
  isEdit?: boolean;
  /** When true, the Cancel/Submit buttons inside the form are hidden.
   *  Use when the parent renders action buttons in a dialog footer. */
  hideActions?: boolean;
  /** Called whenever react-hook-form's isSubmitting state changes.
   *  Lets a parent footer button mirror the loading/disabled state. */
...
export const HabitForm = forwardRef<HTMLFormElement, HabitFormProps>(
  function HabitForm(
    {
      onSubmit,
      onCancel,
    ...
    return (
      <form ref={ref} onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4">
        ...
        {/* Actions — hidden when the parent dialog owns the footer buttons */}
        {!hideActions && (
          <div
